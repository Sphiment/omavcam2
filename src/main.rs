...
fn main() -> ExitCode {
    ...
    match args.as_slice() {
        ...
        other => {
            ...
            eprintln!(
                "usage: omavcam [status|phones|refresh|select <serial>|pair [<pair-address> <code> <connect-address>]|connect <serial> <connect-address>|forget <serial>|start [--stay-awake]|stop|daemon]"
            );
            ExitCode::from(2)
        }
    }
